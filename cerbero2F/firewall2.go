...
func fwFilter(services Services, number int, alertFileEdited chan string, path string) {
	...
	fn := func(a nfqueue.Attribute) int {
		...
		not_managed := true
		log.Println("LUNGHEZZA BYTE", len(*a.Payload))
...
func setRules(services Services, path string) {
	...
	for k := 0; k < len(services.Services); k++ {
		cmd := exec.Command("iptables", "-I", "DOCKER-INGRESS", "-p", services.Services[k].Protocol, "--dport", strconv.FormatInt(int64(services.Services[k].Dport), 10), "-j", "NFQUEUE", "--queue-num", strconv.FormatInt(int64(services.Services[k].Nfq), 10))
		cmd.Run()
	}
	...
	go func() {
		...
		for k := 0; k < len(services.Services); k++ {
			cmd := exec.Command("iptables", "-D", "DOCKER-INGRESS", "-p", services.Services[k].Protocol, "--dport", strconv.FormatInt(int64(services.Services[k].Dport), 10), "-j", "NFQUEUE", "--queue-num", strconv.FormatInt(int64(services.Services[k].Nfq), 10))
			cmd.Run()
		}
